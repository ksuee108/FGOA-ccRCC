        ...
        for particle in self.geese:
            ...
            if neighbors:
                ...
                if all(pos.shape == positions[0].shape for pos in positions):
                    ...
                    particle.velocity += 0.01 * (np.squeeze(avg_position) - particle.position)
                for neighbor in neighbors:
                    if np.linalg.norm(neighbor.position - particle.position) < neighborhood_radius / 2:
                        particle.velocity -= 0.9 * (neighbor.position.reshape(particle.position.shape) - particle.position)
