...
def create_transformer(X_train, X_test, y_train, y_test, opt, input_shape=(10, 15), num_heads=4, ff_dim=64):
    ...
    model.save(f'D:\\張\\TCGA-KIRC-ok\\{opt}\\stage\\{opt} transformer.h5')
...
def cnn(X_train, X_test, y_train, y_test, opt):
    ...
    model.save(f'D:\\張\\TCGA-KIRC-ok\\{opt}\\stage\\{opt} CNN.h5')
    return test_y_predicted, predictions
...
def bert(X_train, X_test, y_train, y_test, opt):
    ...
    training_args = TrainingArguments(
        output_dir=f'D:\\張\\TCGA-KIRC-ok\\{opt}\\stage\\{opt}-bert',
        num_train_epochs=5,
    ...
    # 保存模型
    model.save_pretrained(f'D:\\張\\TCGA-KIRC-ok\\{opt}\\stage\\{opt}-BERT')
    tokenizer.save_pretrained(f'D:\\張\\TCGA-KIRC-ok\\{opt}\\stage\\{opt}-BERT')
...
while(1):
    for model, model_name in lstmmodel:
        ...
        for opt in optimization:
            print(opt)
            path = f'D:\\張\\TCGA-KIRC-ok\\{opt}\\stage'
