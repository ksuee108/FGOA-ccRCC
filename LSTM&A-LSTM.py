...
def a_lstm(X_train, X_test, y_train, y_test, opt):
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            filepath=f'D:\\張\\TCGA-kirc5\\{opt}\\stage\\{opt} A-LSTM.h5',
            monitor='val_loss',
            save_best_only=True,
            verbose=2
        )
    ...
    model2 = load_model(f'D:\\張\\TCGA-kirc5\\{opt}\\stage\\{opt} A-LSTM.h5', custom_objects={'SeqSelfAttention': SeqSelfAttention})
    ...
    # 如果索引是整数型，可以使用 iloc 来获取第一行
    if result_df.iloc[0]['Accuracy'] > result_df2.iloc[0]['Accuracy']:
        model.save(f'D:\\張\\TCGA-kirc5\\{opt}\\stage\\{opt} A-LSTM.h5')
...
def cnn(X_train, X_test, y_train, y_test, opt):
    ...
    model.save(f'D:\\張\\TCGA-kirc5\\{opt}\\stage\\{opt} CNN.h5')
    return test_y_predicted, predictions
...
def create_transformer(X_train, X_test, y_train, y_test, opt, input_shape=(10, 15), num_heads=4, ff_dim=64):
    ...
    model.save(f'D:\\張\\TCGA-kirc5\\{opt}\\stage\\{opt} transformer.h5')
    return test_y_predicted, predictions
...
#'original', 'LASSO', 'ANOVA', "GA", "DE", "PSO", "WOA", "HHO", "SMA", "FGOA"
...
while(1):
    for model, model_name in lstmmodel:
        ...
        for opt in optimization:
            ...
            FULL_X_reshaped = FULL_X.reshape(FULL_X.shape[0], -1)  # Flatten the features for SMOTE
            ...
            X_train_2d = X_test[:, -10, :]  # 將 3D 攤平成 2D
            X_test_2d = X_test[:, -10, :]  # 將 3D 攤平成 2D
            ...
            if model_name != 'Transformer':
                ...
                LSTM_binary, LSTM_predictions = model(X_train, X_test, y_train, y_test, opt, input_shape = (X_train[1].shape))
            
            """
            if model_name != 'A-LSTM':
                model = load_model(f'D:\\張\\TCGA-kirc5\\model\\{opt} {model_name}.h5')
            else:
                model = load_model(f'D:\\張\\TCGA-kirc5\\model\\{opt} {model_name}.h5', custom_objects={'SeqSelfAttention': SeqSelfAttention})
            ...
            if opt == 'FGOA':
                ...
                result = permutation_importance(model, X_test_flat, y_test, n_repeats=30, random_state=42, scoring=custom_score)
            ...
            # 获取混淆结果并存储到 DataFrame
            confusion_result = criteria.confusion(LSTM_binary ,LSTM_predictions, f"{opt} {model_name}")
        ...
        print(final_df['Accuracy'])
        """if 'Accuracy' in final_df.columns and not final_df.empty:  # 確保 'Accuracy' 存在
            if final_df['Accuracy'][0] < 0.80:
                print(final_df['Accuracy'][0])
                continue"""
        ...
        if model_name =="A-LSTM":
            trained_model = load_model(f'D:\\張\\TCGA-kirc5\\model\\FGOA A-LSTM.h5', custom_objects={'SeqSelfAttention': SeqSelfAttention})
            explainer = shap.GradientExplainer (trained_model, X_train)
            shap_values = explainer.shap_values(X_test)
            print(shap_values.shape)
            # 假设 shap_values 是一个列表，包含每个类别的值（二分类问题只有一个列表，表示类别 1 的 SHAP 值）：
            shap_values = shap_values[:,-1,:]  # 如果是二分类，只取第一个元素
            print(shap_values.shape)
            # 重塑 shap_values 使其形状与 X_test_2d 对齐
            shap_values_reshaped = shap_values.reshape(X_test_2d.shape[0], X_test_2d.shape[1])
            print(shap_values_reshaped.shape)

            # 现在生成 SHAP summary plot
